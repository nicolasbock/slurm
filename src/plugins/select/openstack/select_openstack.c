...
#define LOCAL_DEBUG(msg) debug("%s:%s:%d %s", __FILE__, __func__, __LINE__, msg)
...
select_nodeinfo_t *select_p_select_nodeinfo_alloc();
...
static void _add_run_job(struct cr_record *cr_ptr, uint32_t job_id)
{
        ...
        if (cr_ptr->run_job_ids == NULL) {      /* create new array */
                ...
                cr_ptr->run_job_ids =
                    xmalloc(sizeof(uint32_t) * cr_ptr->run_job_len);
...
static void _add_tot_job(struct cr_record *cr_ptr, uint32_t job_id)
{
        ...
        if (cr_ptr->tot_job_ids == NULL) {      /* create new array */
                ...
                cr_ptr->tot_job_ids =
                    xmalloc(sizeof(uint32_t) * cr_ptr->tot_job_len);
        ...
        while (consec_index && (max_nodes > 0) && !heterogeneous) {
                ...
                for (i = 0; i < consec_index; i++) {
                        ...
                        sufficient = (consec_cpus[i] >= rem_cpus) &&
                            _enough_nodes(consec_nodes[i], rem_nodes, min_nodes,
                                          req_nodes);
                ...
                if (job_ptr->details->contiguous &&
                    ((best_fit_cpus < rem_cpus) ||
                     (!_enough_nodes
                      (best_fit_nodes, rem_nodes, min_nodes, req_nodes))))
 ...
 */
static void
_hypercube_update_variance(int dim, int dir, int start_index, int end_index,
                           int node_count, int max_nodes,
                           int leftover_nodes, int64_t summed_squares,
                           int64_t squared_sums, int *min_curve,
                           int *min_direction, int *min_start_index,
                           int *min_neighbors, int *min_extra_nodes,
 ...
 */
static void
_hypercube_add_nodes(struct job_record *job_ptr, bitstr_t * avail_bitmap,
                     int dim, int32_t start_index, int32_t * end_index,
                     int node_count, int32_t max_nodes, int32_t rem_nodes,
                     int32_t rem_cpus, int leftover_nodes, bitstr_t * bitmap,
                     int64_t * distance_offset, int64_t summed_squares,
                     int64_t squared_sums, int *min_curve, int *min_direction,
                     int *min_start_index, int32_t * min_neighbors,
                     int32_t * min_extra_nodes, int64_t * min_variance)
{
        bitstr_t *tmp_bitmap;
        ...
        if (leftover_nodes || (0 >= rem_nodes && 0 >= rem_cpus)) {
                return;
        }
        ...
        while (0 <= l_temp_max_nodes &&
               (0 < l_temp_rem_nodes || 0 < l_temp_rem_cpus)) {
                ...
                if (l_end_index < 0) {  /* Handle wrap-around */
                        ...
                        l_distance_offset = -1 * hypercube_switches[dim]
                ...
                l_leftover_nodes =
                    hypercube_switches[dim][l_end_index]->avail_cnt - new_nodes;
        }

        bit_free(tmp_bitmap);

        /* Let's see how good this right-sided cluster is */
        _hypercube_update_variance(dim, -1, l_start_index, l_end_index,
                                   node_count, l_temp_max_nodes,
                                   l_leftover_nodes, l_summed_squares,
                                   l_squared_sums, min_curve, min_direction,
        ...
        if (*end_index == hypercube_switch_cnt) {       /* Handle wrap-around */
                *end_index = 0;
                *distance_offset =
                    hypercube_switches[dim][hypercube_switch_cnt -
                                            1]->distance[dim];
        }
}
...
_explore_hypercube(struct job_record *job_ptr, bitstr_t * avail_bitmap,
                   ...
                   const int max_nodes,
                   const int rem_nodes, const int rem_cpus,
                   const int node_count, int *min_start_index,
                   int *min_direction, int *min_curve)
{
        ...
        for (dim = 0; dim < hypercube_dimensions; dim++) {
                ...
                if (hypercube_switch_table[0].distance[dim] == 0) {
                        continue;
                }
                       ...
                       && (start_index >= 0)) {
                        ...
                        while ((0 <= temp_max_nodes) &&
                               (0 < temp_rem_nodes || 0 < temp_rem_cpus)) {
                                ...
                                int fn =
                                    hypercube_switches[dim]
                                    [end_index]->node_index[0];
                                int ln =
                                    hypercube_switches[dim]
                                    [end_index]->node_index[cnt - 1];
                                int new_nodes = 0;
                                int n;
                                ...
                                distance = hypercube_switches[dim]
                                    [end_index]->distance[dim] +
                                    distance_offset;
                                summed_squares +=
                                    new_nodes * distance * distance;
                                ...
                                _hypercube_add_nodes(job_ptr, avail_bitmap,
                                                     dim, start_index,
                        ...
                        _hypercube_update_variance(dim, 1, start_index,
                                                   end_index, node_count,
                                                   temp_max_nodes,
                                                   leftover_nodes,
                                                   summed_squares, squared_sums,
                                                   min_curve, min_direction,
                                                   min_start_index,
                                                   &min_neighbors,
                                                   &min_extra_nodes,
                                                   &min_variance);
                        ...
                        while ((temp_rem_nodes <= 0) &&
                               ...
                               && (start_index >= 0)) {
                                int cnt =
                                    hypercube_switches[dim]
                                    [start_index]->node_cnt;
                                int used = MIN(rem_nodes,
                                               hypercube_switches[dim]
                                               [start_index]->avail_cnt);
                                int n;

                                if (hypercube_switches[dim]
                                    [start_index]->avail_cnt == 0) {
                         ...
                         */
                        while ((start_index < hypercube_switch_cnt) &&
                               (hypercube_switches[dim][start_index]->avail_cnt
                                == 0)) {
                                if (start_index == end_index)
                                        end_index++;
                                start_index++;
                               ...
                               uint32_t req_nodes)
{
        ...
        int64_t *req_summed_squares =
            xmalloc(hypercube_dimensions * sizeof(int64_t));
        int64_t *req_squared_sums =
            xmalloc(hypercube_dimensions * sizeof(int64_t));
        ...
        if (job_ptr->details->req_node_bitmap) {
                ...
                if (i > (int)max_nodes) {
                        info("job %u requires more nodes than currently "
                             "available (%d>%u)", job_ptr->job_id, i,
                             max_nodes);
        ...
        for (i = 0; i < hypercube_switch_cnt; i++) {
                ...
                hypercube_switch_table[i].avail_cnt =
                    bit_set_count_range(avail_bitmap, node_idx, node_idx + cnt);
                     ...
                     (req_nodes_bitmap, node_idx, node_idx + cnt) > 0)) {
                        ...
                        for (j = 0; j < cnt; j++) {
                                ...
                                if (bit_test(req_nodes_bitmap, idx)) {
                                         ...
                                         */
                                        for (k = 0; k < hypercube_dimensions;
                                             k++) {
                                                int distance =
                                                    hypercube_switch_table
                                                    [i].distance[k];

                                                req_summed_squares[k] +=
                                                    distance * distance;
        ...
        if ((alloc_nodes >= max_nodes) || ((rem_nodes <= 0) && (rem_cpus <= 0))) {
                goto fini;
        }
        ...
        node_counter = 0;
        while ((alloc_nodes < max_nodes) && ((rem_nodes > 0) || (rem_cpus > 0))) {
                int node_index;
                ...
                if (node_counter ==
                    hypercube_switches[min_curve][switch_index]->avail_cnt) {
                        ...
                        } while (hypercube_switches[min_curve]
                                 [switch_index]->avail_cnt == 0);
                }
                ...
                do {
                        cur_node_index++;
                        node_index =
                            hypercube_switches[min_curve]
                            [switch_index]->node_index[cur_node_index];
                          ...
                          uint32_t req_nodes)
{
        ...
        if (job_ptr->details->req_node_bitmap) {
                ...
                if (i > (int)max_nodes) {
                        info("job %u requires more nodes than currently "
                             "available (%u>%u)", job_ptr->job_id, i,
                             max_nodes);
                        rc = EINVAL;
                        goto fini;
                }
        ...
        for (i = 0; i < switch_record_cnt; i++) {
                ...
                if (req_nodes_bitmap &&
                    !bit_super_set(req_nodes_bitmap, switches_bitmap[i]))
                        ...
                        switches_node_cnt[i] =
                            bit_set_count(switches_bitmap[i]);
                        sufficient = true;
                }
        }
        bit_nclear(bitmap, 0, node_record_count - 1);
        ...
        for (j = 0; j < switch_record_cnt; j++) {
                if ((switch_record_table[j].level > 0) ||
                    (!bit_super_set(switches_bitmap[j],
                        ...
                        switches_node_cnt[j] =
                            bit_set_count(switches_bitmap[j]);
                }
        }
               ...
               ((alloc_nodes < want_nodes) || (rem_cpus > 0))) {
                ...
                if (job_ptr->req_switch > 0) {
                        if (time_waiting >= job_ptr->wait4switch) {
                                ...
                                debug3
                                    ("Job=%u Waited %ld sec for switches use=%d",
                       ...
                       struct job_record *to_job_ptr)
{
        ...
        new_job_resrcs_ptr->nodes =
            bitmap2node_name(new_job_resrcs_ptr->node_bitmap);
        ...
        for (i = first_bit; i <= last_bit; i++) {
                ...
                if (from_node_used) {
                        ...
                        new_job_resrcs_ptr->memory_allocated[new_node_offset] =
                            from_job_resrcs_ptr->memory_allocated
                            [from_node_offset];
                ...
                if (to_node_used) {
                        ...
                        new_job_resrcs_ptr->memory_allocated[new_node_offset] +=
                            to_job_resrcs_ptr->memory_allocated[to_node_offset];
                        new_job_resrcs_ptr->memory_used[new_node_offset] +=
                                 ...
                                 struct node_record *node_ptr, char *pre_err)
{
        ...
        if (!bit_test(job_resrcs_ptr->node_bitmap, node_inx)) {
                error("job %u allocated nodes (%s) which have been removed "
                      "from slurm.conf", job_ptr->job_id, node_ptr->name);
                return SLURM_ERROR;
        }
        ...
        if (cr_ptr->nodes[node_inx].alloc_memory >= job_memory)
                ...
                error("%s: memory underflow for node %s", pre_err,
                      node_ptr->name);
        }
...
static struct cr_record *_dup_cr(struct cr_record *cr_ptr)
{
        ...
        for (i = 0; i < select_node_cnt; i++) {
                ...
                while (part_cr_ptr) {
                        ...
                        new_part_cr_ptr->next = new_cr_ptr->nodes[i].parts;
                ...
                if (job_ptr->details && job_ptr->details->pn_min_memory &&
                    (cr_type & CR_MEMORY)) {
                        if (job_ptr->details->pn_min_memory & MEM_PER_CPU) {
                                job_memory_cpu =
                                    job_ptr->
                                    details->pn_min_memory & (~MEM_PER_CPU);
                        } else {
                                job_memory_node =
                                    job_ptr->details->pn_min_memory;
                        }
                    ...
                    List preemptee_candidates, List * preemptee_job_list)
{
        ...
        for (max_run_job = 0;
             ((max_run_job < max_share) && (rc != SLURM_SUCCESS));
             max_run_job++) {
            ...
            (exp_cr = _dup_cr(cr_ptr))) {
                ...
                while ((tmp_job_ptr = (struct job_record *)
                        ...
                        _rm_job_from_nodes(exp_cr, tmp_job_ptr, "_run_now",
                                           remove_all);
                        j = _job_count_bitmap(exp_cr, job_ptr, orig_map, bitmap,
                ...
                if ((rc == SLURM_SUCCESS) && preemptee_job_list
                    && preemptee_candidates) {
                        ...
                        preemptee_iterator =
                            list_iterator_create(preemptee_candidates);
                        while ((tmp_job_ptr = (struct job_record *)
                                list_next(preemptee_iterator))) {
                                if (bit_overlap
                                    (bitmap, tmp_job_ptr->node_bitmap) == 0)
                                        continue;
                          ...
                          List preemptee_candidates, List * preemptee_job_list)
{
        ...
        if ((rc == SLURM_SUCCESS) && preemptee_job_list && preemptee_candidates) {
                /* Build list of preemptee jobs whose resources are
                      ...
                      List * preemptee_job_list, bitstr_t * exc_core_bitmap)
{
        ...
        if (job_ptr->details->core_spec != (uint16_t) NO_VAL) {
                verbose("select/openstack: job %u core_spec(%u) not supported",
...
int select_p_job_begin(struct job_record *job_ptr)
{
        ...
        if (cr_ptr == NULL)
                _init_node_cr();
        if (rc == SLURM_SUCCESS)
                rc = _add_job_to_nodes(cr_ptr, job_ptr, "select_p_job_begin",
                                       1);
...
bool select_p_job_expand_allow(void)
{
        LOCAL_DEBUG("starting");
...
bitstr_t *select_p_step_pick_nodes(struct job_record * job_ptr,
                                   ...
                                   uint32_t node_count, bitstr_t ** avail_nodes)
{
        LOCAL_DEBUG("starting");
...
int select_p_pack_select_info(time_t last_query_time,
...
int select_p_select_nodeinfo_pack(select_nodeinfo_t * nodeinfo,
...
int select_p_select_nodeinfo_unpack(select_nodeinfo_t ** nodeinfo,
                                    Buf buffer, uint16_t protocol_version)
{
...
int select_p_select_nodeinfo_set_all(void)
{
        ...
        if (last_set_all && (last_node_update < last_set_all)) {
                debug2("Node select info for set all hasn't "
                       "changed since %ld", (long)last_set_all);
 ...
 * NOTE: returned value must be freed using select_p_select_jobinfo_free
 */
int select_p_select_jobinfo_unpack(select_jobinfo_t ** jobinfo,
                                   Buf buffer, uint16_t protocol_version)
{
        LOCAL_DEBUG("starting");
...
char *select_p_select_jobinfo_sprint(select_jobinfo_t * jobinfo,
                                     char *buf, size_t size, int mode)
{
        LOCAL_DEBUG("starting");

        if (buf && size) {
                strncpy(buf, "**** jobinfo ***", size);
                return buf;
        } else
                return NULL;
}
...
int select_p_alter_node_cnt(enum select_node_cnt type, void *data)
{
        ...
        case SELECT_GET_NODE_SCALING:
                if ((*nodes) != (uint32_t) INFINITE) {
                        LOCAL_DEBUG("setting scaling factor to 1");
                        *nodes = 1;
                }
                break;
        case SELECT_GET_NODE_CPU_CNT:
                if ((*cpus) != (uint16_t) INFINITE) {
                        LOCAL_DEBUG("setting CPUs to 10");
                        *cpus = 10;
                }
                break;
        case SELECT_GET_MP_CPU_CNT:
                if ((*nodes) != (uint32_t) INFINITE) {
                        LOCAL_DEBUG("setting CPUs per partition to 1");
                        (*nodes) = 1;
                }
                break;
        case SELECT_APPLY_NODE_MIN_OFFSET:
                LOCAL_DEBUG("applying min offset of 0");
                break;
        case SELECT_APPLY_NODE_MAX_OFFSET:
                LOCAL_DEBUG("applying max offset of 0");
                break;
        case SELECT_SET_NODE_CNT:
                LOCAL_DEBUG("node count is not altered");
                break;
        case SELECT_SET_MP_CNT:
                LOCAL_DEBUG("partition count is not altered");
                break;
        default:
...
bitstr_t *select_p_resv_test(resv_desc_msg_t * resv_desc_ptr,
                             uint32_t node_cnt,
                             bitstr_t * avail_bitmap, bitstr_t ** core_bitmap)
{
        ...
        for (i = 0; i < switch_record_cnt; i++) {
                ...
                info("switch=%s level=%d nodes=%u:%s required:%u speed=%u",
                     ...
                     switches_node_cnt[i], node_names,
                     switches_required[i], switch_record_table[i].link_speed);
                xfree(node_names);
        }
#endif
